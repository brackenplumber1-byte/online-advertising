...
<?php while(have_posts()): the_post();
  ...
  $response_time = gp_area_response_time($slug);
  ...
  $nearby_areas = gp_nearby_areas($slug, $areas, 8);
...
<div class="page-hero">
  ...
  <div class="container">
    ...
    <p>Searching for a plumber near me in <?php echo esc_html($name); ?>? Fast, reliable, PIRB-registered plumbers serving <?php echo esc_html($name); ?> and surrounding areas. Available 24/7 — no call-out fee, on-site within <?php echo esc_html($response_time); ?>.</p>
...
<div class="container" style="padding-top:32px">
  <img src="<?php echo esc_url($imgdir . $area_photo); ?>" alt="Mondeor Plumbing Services — plumber in <?php echo esc_attr($name); ?>" style="width:100%;max-height:380px;object-fit:cover;border-radius:8px">
</div>
...
<div class="container">
  <div class="content-grid">
    <div class="content-body">
      ...
      <p>Mondeor Plumbing Services provides fast, professional plumbing services to residents and businesses in <?php echo esc_html($name); ?>. Based right here in Mondeor, we can reach <?php echo esc_html($name); ?> quickly — making us one of the fastest-responding plumbing companies in the area.</p>
      ...
      <ul>
        <li>Based in Mondeor — fast response to <?php echo esc_html($name); ?></li>
        ...
        <li>Over 10 years serving Mondeor and Johannesburg South</li>
      </ul>
      ...
      <div style="background:var(--off);border-left:4px solid var(--red);padding:20px 24px;margin:28px 0;border-radius:0 4px 4px 0">
        ...
        <p style="margin:0">Call <strong><a href="<?php echo esc_attr(gp_phone_link()); ?>" style="color:var(--red)"><?php echo esc_html(gp_phone()); ?></a></strong> — 24/7, no call-out fee, on-site within <?php echo esc_html($response_time); ?>.</p>
      </div>

      <h2>Frequently Asked Questions — Plumber in <?php echo esc_html($name); ?></h2>
      <?php
      $area_faqs = [
          ['Is there a plumber near me in ' . $name . '?', 'Yes — we\'re based nearby in Mondeor and cover ' . $name . ' directly, so if you\'re searching "plumber near me" from ' . $name . ', we aim to have someone on-site within ' . $response_time . '.'],
          ['How much does a plumber cost in ' . $name . '?', 'It depends on the job. We give a free, no-obligation quote before starting any work in ' . $name . ', and we never charge a call-out fee — you only pay for the work done.'],
          ['Do you offer same-day service in ' . $name . '?', 'In most cases, yes. Burst pipes, blocked drains and geyser failures in ' . $name . ' are treated as emergencies and prioritised for same-day attention.'],
          ['Are your ' . $name . ' plumbers PIRB registered?', 'Yes. Every plumber we send to ' . $name . ' is PIRB-registered, and we issue compliance certificates (CoC) for geyser installations and other qualifying work.'],
      ];
      ?>
      <div class="faq-list">

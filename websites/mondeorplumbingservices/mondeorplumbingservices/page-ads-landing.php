 ...
 * Mondeor Plumbing Services — minimal page for Google Ads traffic.
 * No nav, no footer links, no dropdowns — just the offer, phone, WhatsApp, form.
 *
 * To use: create a new WordPress Page, set its Template to
 * "Ads Landing Page (Distraction-Free)" in the Page Attributes panel,
 * then point your Google Ads final URL at that page.
 *
...
<style>
  ...
  body.ads-landing { margin:0; font-family:'Inter',sans-serif; background:var(--off,#F4F5F7); color:var(--navy,#0A1628); }
  .al-wrap { max-width: 720px; margin: 0 auto; padding: 32px 24px 64px; }
  .al-topbar { background:var(--navy,#0A1628); color:#fff; text-align:center; padding:12px 16px; font-size:.85rem; font-weight:700; }
  .al-topbar a { color:#5EEAD4; }
  .al-logo { display:flex; align-items:center; justify-content:center; gap:10px; margin:28px 0 20px; }
  .al-logo img { width:48px; height:48px; border-radius:8px; }
  .al-logo span { font-weight:800; font-size:1.2rem; }
  .al-hero { text-align:center; margin-bottom:32px; }
  .al-hero h1 { font-family:'Bebas Neue',sans-serif; font-size:clamp(2rem,6vw,2.8rem); line-height:1.1; color:var(--navy,#0A1628); margin-bottom:12px; }
  .al-hero h1 span { color:var(--red,#C8102E); }
  .al-hero p { color:#64748B; font-size:1.05rem; max-width:520px; margin:0 auto; }
  .al-trust { display:flex; flex-wrap:wrap; justify-content:center; gap:10px; margin:24px 0 32px; }
  .al-trust span { background:#fff; border:1px solid #E5E7EB; border-radius:20px; padding:7px 16px; font-size:.78rem; font-weight:700; color:#0A1628; }
  .al-cta-row { display:flex; flex-wrap:wrap; justify-content:center; gap:12px; margin-bottom:40px; }
  .al-btn { display:inline-flex; align-items:center; gap:10px; padding:18px 32px; border-radius:6px; font-weight:800; font-size:1.05rem; text-decoration:none; }
  .al-btn svg { width:22px; height:22px; fill:#fff; }
  .al-btn-call { background:var(--red,#C8102E); color:#fff; }
  .al-btn-wa { background:#25D366; color:#fff; }
  .al-form-card { background:#fff; border-radius:10px; box-shadow:0 8px 30px rgba(0,0,0,.08); padding:32px; }
  .al-form-card h2 { font-size:1.3rem; margin-bottom:6px; }
  .al-form-card > p { color:#64748B; font-size:.9rem; margin-bottom:20px; }
  .al-form-group { margin-bottom:14px; }
  .al-form-group label { display:block; font-size:.72rem; font-weight:700; text-transform:uppercase; letter-spacing:.06em; color:#0A1628; margin-bottom:5px; }
  .al-form-group input, .al-form-group select { width:100%; padding:12px 14px; border:1.5px solid #E5E7EB; border-radius:6px; font-size:.92rem; font-family:inherit; }
  .al-form-submit { width:100%; background:var(--red,#C8102E); color:#fff; border:none; padding:16px; border-radius:6px; font-weight:800; font-size:1rem; cursor:pointer; letter-spacing:.02em; text-transform:uppercase; }
...
<div class="al-topbar">🚨 Plumbing Emergency? <a href="<?php echo esc_attr($phone_link); ?>">Call <?php echo esc_html($phone); ?> now</a> — available 24/7</div>

<div class="al-wrap">

  <div class="al-logo">
    <img src="<?php echo esc_url($imgdir . 'logo.png'); ?>" alt="Mondeor Plumbing Services">
    <span>Mondeor Plumbing Services</span>
  </div>

  <div class="al-hero">
    <h1>Plumber Near You, <span>Anywhere in Johannesburg South.</span></h1>
    <p>24/7 emergency plumbing, geyser repairs, drain cleaning and leak detection — PIRB registered, no call-out fee, fast response.</p>
  </div>
      ...
      <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
        ...
        <div class="al-form-group">
          ...
          <select name="gp_service">
            <?php $services = unserialize(GP_SERVICES); foreach ($services as $v): ?>
  ...
  <div class="al-reviews">
    <div class="stars">★★★★★</div>
    <p>4.9★ rated on Google — trusted by homeowners across Mondeor and Johannesburg South</p>
  </div>

  <p class="al-footer-note">Mondeor Plumbing Services · 10 Boswell Ave, Mondeor, Johannesburg South, 2095 · PIRB Registered</p>

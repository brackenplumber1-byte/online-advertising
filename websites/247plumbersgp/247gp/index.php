...
<section style="background:#fff;padding:48px 0">
  <div class="container">
    ...
    <div class="photo-strip-grid">
      ...
      <img src="<?php echo esc_url($imgdir . 'geyser-branded.webp'); ?>" alt="247 Plumbers GP geyser repair technician">
      <img src="<?php echo esc_url($imgdir . 'water-meter-install.webp'); ?>" alt="247 Plumbers GP water meter and pressure valve installation">
      <img src="<?php echo esc_url($imgdir . 'pipe-branded.webp'); ?>" alt="247 Plumbers GP pipe repair technician">
      <img src="<?php echo esc_url($imgdir . 'geyser-solar-roof.webp'); ?>" alt="247 Plumbers GP solar geyser installation">
    </div>
  </div>
</section>

<!-- ═══ PRIORITY LINKS ══════════════════════════════════════════════════════
     Deliberately gives extra homepage link weight to a small set of pages
     (Emergency Plumbing + 3 flagship areas) rather than treating every
     service and area with identical prominence. The homepage is normally
     the highest-authority page on the site, so what it links to — and how
     many times — matters more than any other single page's links. -->
<section style="background:var(--off);padding:40px 0">
  <div class="container">
    <div class="eyebrow">Most Requested</div>
    <h2 class="display" style="font-size:clamp(1.4rem,2.6vw,1.9rem);color:var(--navy);margin:6px 0 20px">Start Here</h2>
    <?php
    $priority_links = [];
    $emerg_post = get_posts(['name'=>'emergency-plumbing','post_type'=>'gp_service','post_status'=>'publish','numberposts'=>1])[0] ?? null;
    $priority_links[] = ['label'=>'24/7 Emergency Plumbing', 'url'=>$emerg_post ? get_permalink($emerg_post->ID) : home_url('/services/emergency-plumbing')];
    foreach (['midrand'=>'Plumber in Midrand', 'sandton'=>'Plumber in Sandton', 'centurion'=>'Plumber in Centurion'] as $area_slug => $label) {
        $p = get_posts(['name'=>$area_slug,'post_type'=>'gp_area','post_status'=>'publish','numberposts'=>1])[0] ?? null;
        $priority_links[] = ['label'=>$label, 'url'=>$p ? get_permalink($p->ID) : home_url('/areas/'.$area_slug)];
    }
    ?>
    <div style="display:flex;flex-wrap:wrap;gap:12px">
      <?php foreach ($priority_links as $pl): ?>
      <a href="<?php echo esc_url($pl['url']); ?>" style="background:#fff;border:1px solid var(--border);border-radius:8px;padding:12px 20px;font-weight:700;font-size:.92rem;color:var(--navy);text-decoration:none;box-shadow:0 1px 3px rgba(0,0,0,.04)"><?php echo esc_html($pl['label']); ?> →</a>
      <?php endforeach; ?>
    </div>
  </div>
</section>
    ...
    <div class="why-2col">
      <div>
        ...
        <div>
          ...
          <div class="why-row"><div class="why-n">03</div><div class="why-c"><h3>30-Minute Local Response</h3><p>Based in Midrand, we reach most of Gauteng fast for emergencies.</p></div></div>
...
<section id="areas" class="section section--dark">
  <div class="container">
    ...
    <h2 class="display" style="font-size:clamp(2rem,4vw,3.4rem);color:#fff;margin-top:8px">Serving Midrand<br>&amp; All of Gauteng.</h2>
...
<section id="articles" class="section" style="background:var(--off)">
  <div class="container">
    ...
    <p style="max-width:600px;margin:14px 0 36px">Practical plumbing advice for Midrand, Johannesburg and the rest of Gauteng — written by our own team.</p>
...
<section id="faq" class="section">
  <div class="container" style="max-width:800px">
    ...
    <div class="faq-list">
      ...
      <div class="faq-item"><div class="faq-q" onclick="toggleFaq(this)">How fast can you respond to an emergency?<div class="faq-icon"><svg viewBox="0 0 24 24"><path d="M7 10l5 5 5-5z"/></svg></div></div><div class="faq-a">We aim to be on-site within 30 minutes for emergencies in and around Midrand, and as fast as possible across our wider Gauteng coverage area. Being local is our biggest advantage.</div></div>
    ...
    <div class="contact-2col">
      <div>
        ...
        <div>
          ...
          <div class="cm"><div class="cm-i"><svg width="19" height="19" fill="#F87171" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg></div><div><div class="cm-l">Location</div><div class="cm-v" style="color:#CBD5E1">Silkwood Complex, Berger Rd, Vorna Valley, Midrand, 1686</div></div></div>
      ...
      <div>
        <?php if($maps): ?>
        <div class="map-wrap">
          <iframe src="<?php echo esc_url($maps); ?>" allowfullscreen loading="lazy" title="247 Plumbers GP service area"></iframe>
        </div>
        <?php else: ?>
        <div class="map-wrap">
          <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d57339.5!2d28.1267!3d-25.9892!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x1e9569c1d4b8b1f1%3A0x0!2sMidrand%2C+Gauteng!5e0!3m2!1sen!2sza!4v1700000000000" allowfullscreen loading="lazy" title="247 Plumbers GP location — Midrand"></iframe>
        </div>
        <?php endif; ?>
        <div class="cf-box">

...
description: |
  ...
  copy-paste into wp-admin needed. Use this whenever the user asks to
  "post this to the website", "update the WordPress site", "publish a
  blog post/article", "add a page", "upload this image to the site", or
  wants a scheduled/regular cadence of website content published to
...
1. **Never invent credentials or guess a site URL.** If `.env` isn't
   set up yet, walk the user through the one-time setup below. Don't
   ask for the actual password to be pasted in chat if you can avoid
   it — prefer they create the `.env` file themselves, or paste it
   directly into a file you write (not into a message that becomes
   part of a shared transcript) if that's the only option available.
2. **Default new posts to `draft` status, not `publish`.** Only pass
   ...
   established.
3. **Read before you write.** Before updating an existing post/page,
...
4. **Match the site's existing voice and structure.** Before drafting
   ...
   AI-written article.
5. **`RESULT:` lines are the canonical channel.** Every `wp_client.py`
...
1. **Generate an Application Password**: in wp-admin, go to **Users →
   Profile** (or **Users → All Users → [user] → Edit**), scroll to
   **Application Passwords**, enter a name like "Claude publishing",
   click **Add New Application Password**. WordPress shows the
...
3. **Configure credentials**: copy `.env.example` to `.env` and fill in
   `WP_SITE_URL`, `WP_USERNAME`, and `WP_APP_PASSWORD` (the space-
   separated password WordPress showed you — keep the spaces, they're
   part of it). Never commit `.env`.
4. **Verify**:
   ```bash
   python3 scripts/wp_client.py whoami
   ```
...
python3 scripts/wp_client.py create-post \
...
```bash
python3 scripts/wp_client.py whoami
python3 scripts/wp_client.py list-posts --status draft --limit 10
python3 scripts/wp_client.py get-post --id 42
python3 scripts/wp_client.py create-post --title "..." --content-file /tmp/x.html --status draft
python3 scripts/wp_client.py update-post --id 42 --status publish
python3 scripts/wp_client.py upload-media --file image.jpg --alt-text "..."
python3 scripts/wp_client.py list-categories
```

Credentials: `.env` in this skill's directory (`WP_SITE_URL`,
`WP_USERNAME`, `WP_APP_PASSWORD`) — never commit it.
